...
high = np.array([0.1, 0.1, 0.1, 0.1, 30])
...
high = np.array([85], dtype=np.float32)
...
for episode in range(500):
    ...
    while t < tot_time:
        ...
        agent.memory.push(x0, action, reward, new_obs_noise)  # measurment noise
        if len(agent.memory) > batch_size:
            agent.train(6, batch_size)
        t = t + dt
        x0 = new_obs_noise  # measurement noise
        lo += (np.abs(new_state_noise_2 - 0.143) ** 2)  # measurment noise
    ...
    CSTR_TD3_noise.append(propylene_glycol)

np.savetxt("C:\\Users\\Nikita\\Objective 3\\CSTR_noise\\CSTR_TD3_noise.csv", CSTR_TD3_noise, delimiter=",")
