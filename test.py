...
def test():
    ...
    state_clip_low = np.array(observation_space.high)
    state_clip_high = np.array(observation_space.low)
    max_goal = np.array([0.01450])
    ...
    exploration_action_noise = np.array([0.5])
    exploration_state_noise = np.array([0.5])
    ...
    n_iter = 6  # update policy n_iter times in one DDPG update
    batch_size = 5  # num of transitions sampled from replay buffer
    lr = 0.001
    ...
    agent.rewards = np.zeros(max_episodes)
    ...
    for i_episode in range(1, max_episodes + 1):
        ...
        plot_G(agent.propylene_glycol, tot_time, agent.flowrate)
        # update all levels
        agent.rewards[i_episode - 1] = agent.reward
